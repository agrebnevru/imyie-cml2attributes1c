...
class CIMYIECML2Attr
{
	function OnAfterIBlockElementAddUpdateHandler(&$arFields)
	{
		if(CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog'))
		{
			...
			if($arCatalog && is_array($arCatalog) && IntVal($arCatalog["PRODUCT_IBLOCK_ID"])>0 && IntVal($ELEMENT_ID)>0)
			{
				$dbRes = CIBlockProperty::GetList(array(),array("ACTIVE"=>"Y","IBLOCK_ID"=>$IBLOCK_ID,"CODE"=>$CML2_PROP_CODE));
